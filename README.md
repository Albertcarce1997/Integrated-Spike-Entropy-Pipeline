...
| seqtk    | 1.5      | FASTQ-to-FASTA conversion                                     |
| VSEARCH  | 2.28.1   | Sequence clustering and reference search                      |
...
The pipeline processes each ARTIC primer scheme (`ARTIC_3`, `ARTIC_4`, `ARTIC_5`) sequentially through six stages, followed by a final summary step:
...
Each ARTIC version defines different amplicon boundaries. ARTIC v3 covers amplicons A71–A84; ARTIC v4 covers amplicons A72–A84; ARTIC v5 covers amplicons A69–A82.

### Step 2 — Sequence clustering and reference search

For each amplicon FASTQ:

1. **FASTQ-to-FASTA conversion**: seqtk converts reads from FASTQ to FASTA format.
...
2. **Position mapping**: The amplicon amino acid sequence is aligned to the Wuhan-Hu-1 Spike amino acid reference (`wuhan_AA_Spike.fasta`) using MAFFT (`--localpair --maxiterate 1000`, OP=12, EP=3) to establish a mapping from amplicon positions to Wuhan Spike positions.
3. **MPLF insertion trimming (ARTIC v5 / A70 only)**: Before alignment, all occurrences of the 4-amino-acid motif `MPLF` are removed from each query read. This insertion falls at the beginning of amplicon A70 in ARTIC v5 and causes systematic alignment artifacts if left in place. No other amplicon or ARTIC version is affected.
4. **Per-read alignment**: Each query amino acid sequence is individually aligned to the amplicon amino acid reference using MAFFT (`--localpair --maxiterate 1000`, OP=12, EP=3). Heuristics detect and correct pathological head and tail alignment artifacts where a few amino acids are misplaced across a long deletion gap.

   > **Speed note:** MAFFT can alternatively be run with `--auto` mode for significantly faster execution. Testing on a small subset of samples produced highly similar results. However, the default pipeline uses L-INS-i (`--localpair`) for maximum accuracy.

5. **Matrix construction**: A wide matrix is built with one row per read and one column per Wuhan Spike reference position. Cells contain the query amino acid at that position, `del` for deletions, or empty for positions outside the read coverage.
6. **Entropy calculation**: Per-position Shannon entropy is computed using Dirichlet smoothing (α = 0.01 applied only to observed amino acids) and normalized by log₂(min(21, *N*)) where *N* is the total weighted read depth at the position. 
7. **Output generation**: Per-sample Excel files with the full amino acid matrix and per-sample entropy CSV files are generated. A combined Excel file aggregates entropy values across all samples for each amplicon.

### Step 7 — Cross-amplicon entropy summary

After all ARTIC versions have been processed, the pipeline merges the per-amplicon `combined_entropy_by_sample.xlsx` files into a single summary workbook (`per_position_entropy.xlsx`) in the pipeline root directory. The workbook contains one sheet per ARTIC version (`ARTIC_3`, `ARTIC_4`, `ARTIC_5`), with each row representing a sample and each column a Wuhan Spike amino acid position.

For every sample, three metadata columns are produced:

| Column        | Description                                                                  |
| ------------- | ---------------------------------------------------------------------------- |
| `Sample_ID` | The original identifier extracted from the input FASTQ filename              |
| `Sample`    | A shortened label derived from the sample identifier (see patterns below)    |
| `Date`      | A sampling date in `YYYY-MM-DD` format, if one can be parsed from the name |

#### Recognised sample naming patterns

The pipeline attempts to parse the `Sample_ID` to extract a short label and a date. The following naming conventions are recognised (tried in order):

| Pattern                        | Example                   | Sample       | Date           |
| ------------------------------ | ------------------------- | ------------ | -------------- |
| `YYMMDD-EDAR_NN-X`           | `230115-EDAR_42-A`      | `42`       | `2023-01-15` |
| `YYMMDD_EDAR_NN`             | `230115_EDAR_42`        | `42`       | `2023-01-15` |
| `YYMMDD-EDAR_NN`             | `230115-EDAR_42`        | `42`       | `2023-01-15` |
| `<word>-YYYY-MM-DD`          | `SiteA-2023-01-15`     | `SiteA`    | `2023-01-15` |
| Any name containing `YYMMDD` | `barcode12_230115_run3` | (full ID)    | `2023-01-15` |

> **Fallback behaviour.** If the sample identifier does not match any of the patterns above, the pipeline **will not fail**. The `Sample` column will contain the full original identifier and the `Date` column will be left empty. This ensures that samples with non-standard naming conventions are still included in the summary output.
...
Additionally, a cross-amplicon summary file is created in the pipeline root directory:

```
pipeline_directory/
└── per_position_entropy.xlsx
```
...
| `*_entropy.csv`                   | Per-sample CSV file with Dirichlet-smoothed Shannon entropy, most common amino acid, and coverage at each position                |
...
| `per_position_entropy.xlsx`       | Final summary workbook (pipeline root) with one sheet per ARTIC version, merging all amplicons per sample with sample metadata     |
...
| Dirichlet smoothing α     | 0.01          | Pseudo-count added to each observed amino acid for entropy |
| Entropy normalization      | min(21,*N*) | Maximum effective diversity used as log₂ denominator      |
| MPLF trimming              | ARTIC v5 A70  | 4-AA insertion motif removed before alignment              |
| MAFFT gap penalties        | OP=12, EP=3   | Gap open and extension penalties for pairwise alignments   |
